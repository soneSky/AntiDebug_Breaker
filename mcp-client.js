...
function initMCPConnection() {
    ...
    chrome.storage.local.get(['mcp_enabled', 'mcp_port'], (result) => {
        ...
        console.log(`[MCP Client] initMCPConnection: mcpEnabled=${mcpEnabled}, mcpPort=${mcpPort}`);

        if (mcpEnabled) {
            console.log('[MCP Client] MCP已启用，准备连接服务器...');
            connectToMCP();
        } else {
            console.log('[MCP Client] MCP未启用，跳过连接');
        }
    });
}
...
function startHeartbeat() {
    ...
    heartbeatInterval = setInterval(() => {
        if (!ws || ws.readyState !== WebSocket.OPEN) {
            console.log('[MCP Client] 心跳检测：连接已断开，停止心跳');
            handleDisconnect();
            return;
        }
        ...
        if (timeSinceLastPong > HEARTBEAT_TIMEOUT) {
            console.log(`[MCP Client] 心跳超时: ${timeSinceLastPong}ms > ${HEARTBEAT_TIMEOUT}ms，连接可能已断开`);
            handleDisconnect();
            // 尝试重连
            if (mcpEnabled && reconnectAttempts < MAX_RECONNECT_ATTEMPTS) {
                reconnectAttempts++;
                console.log(`[MCP Client] 心跳超时后重连 (${reconnectAttempts}/${MAX_RECONNECT_ATTEMPTS})`);
                setTimeout(connectToMCP, RECONNECT_INTERVAL);
            }
            return;
        }
        ...
        // 发送心跳
        try {
            console.log(`[MCP Client] 发送心跳 PING, 距上次pong=${timeSinceLastPong}ms`);
            ws.send(JSON.stringify({ type: 'PING', timestamp: Date.now() }));
        } catch (e) {
            console.error('[MCP Client] 发送心跳失败:', e.message);
            handleDisconnect();
        }
    }, HEARTBEAT_INTERVAL);
}
...
async function handleMCPMessage(message) {
    ...
    console.log(`[MCP Client] handleMCPMessage: type=${type}, requestId=${requestId}, data=${data ? JSON.stringify(data).substring(0, 100) : 'none'}`);
    ...
    try {
        ...
        switch (type) {
            case 'GET_PAGE_INFO':
                console.log('[MCP Client] 执行 GET_PAGE_INFO');
                result = await getPageInfo();
                break;

            case 'GET_NETWORK_REQUESTS':
                console.log(`[MCP Client] 执行 GET_NETWORK_REQUESTS, filter=${data ? data.filter : 'none'}, limit=${data ? data.limit : 50}`);
                result = await getNetworkRequests(data);
                break;

            case 'GET_VUE_ROUTER_DATA':
                console.log('[MCP Client] 执行 GET_VUE_ROUTER_DATA');
                result = await getVueRouterData();
                break;

            case 'GET_HOOK_DATA':
                console.log(`[MCP Client] 执行 GET_HOOK_DATA, scriptId=${data ? data.scriptId : 'all'}, limit=${data ? data.limit : 50}`);
                result = await getHookData(data);
                break;

            case 'GET_ENABLED_SCRIPTS':
                console.log('[MCP Client] 执行 GET_ENABLED_SCRIPTS');
                result = await getEnabledScripts();
                break;

            case 'TOGGLE_SCRIPT':
                console.log(`[MCP Client] 执行 TOGGLE_SCRIPT, scriptId=${data.scriptId}, enabled=${data.enabled}`);
                result = await toggleScript(data);
                break;

            case 'NAVIGATE_TO':
                console.log(`[MCP Client] 执行 NAVIGATE_TO, url=${data.url}`);
                result = await navigateTo(data);
                break;

            case 'GET_COOKIES':
                console.log(`[MCP Client] 执行 GET_COOKIES, name=${data ? data.name : 'all'}`);
                result = await getCookies(data);
                break;

            case 'GET_LOCAL_STORAGE':
                console.log(`[MCP Client] 执行 GET_LOCAL_STORAGE, key=${data ? data.key : 'all'}`);
                result = await getLocalStorage(data);
                break;

            case 'GET_SESSION_STORAGE':
                console.log(`[MCP Client] 执行 GET_SESSION_STORAGE, key=${data ? data.key : 'all'}`);
                result = await getSessionStorage(data);
                break;

            case 'EXECUTE_SCRIPT':
                console.log(`[MCP Client] 执行 EXECUTE_SCRIPT, codeLength=${data.code ? data.code.length : 0}, world=${data.world || 'MAIN'}`);
                result = await executeScript(data);
                break;

            case 'GET_DOM_INFO':
                console.log(`[MCP Client] 执行 GET_DOM_INFO, selector=${data.selector}, depth=${data.depth}`);
                result = await getDOMInfo(data);
                break;

            case 'CONFIGURE_HOOK':
                console.log(`[MCP Client] 执行 CONFIGURE_HOOK, scriptId=${data.scriptId}`);
                result = await configureHook(data);
                break;

            case 'LIST_AVAILABLE_SCRIPTS':
                console.log(`[MCP Client] 执行 LIST_AVAILABLE_SCRIPTS, category=${data.category || 'all'}`);
                result = await listAvailableScripts(data);
                break;

            case 'REFRESH_PAGE':
                console.log(`[MCP Client] 执行 REFRESH_PAGE, hardRefresh=${data.hardRefresh}`);
                result = await refreshPage(data);
                break;

            // ===== 新增：浏览器控制功能 =====
            case 'TAKE_SCREENSHOT':
                console.log(`[MCP Client] 执行 TAKE_SCREENSHOT, fullPage=${data.fullPage}`);
                result = await takeScreenshot(data);
                break;

            case 'CLICK_ELEMENT':
                console.log(`[MCP Client] 执行 CLICK_ELEMENT, selector=${data.selector}`);
                result = await clickElement(data);
                break;

            case 'FILL_INPUT':
                console.log(`[MCP Client] 执行 FILL_INPUT, selector=${data.selector}`);
                result = await fillInput(data);
                break;

            case 'PRESS_KEY':
                console.log(`[MCP Client] 执行 PRESS_KEY, key=${data.key}`);
                result = await pressKey(data);
                break;

            case 'GET_CONSOLE_MESSAGES':
                console.log(`[MCP Client] 执行 GET_CONSOLE_MESSAGES, limit=${data.limit}`);
                result = await getConsoleMessages(data);
                break;

            case 'SCROLL_PAGE':
                console.log(`[MCP Client] 执行 SCROLL_PAGE, x=${data.x}, y=${data.y}`);
                result = await scrollPage(data);
                break;

            case 'WAIT_FOR_SELECTOR':
                console.log(`[MCP Client] 执行 WAIT_FOR_SELECTOR, selector=${data.selector}`);
                result = await waitForSelector(data);
                break;

            case 'GET_ELEMENT_INFO':
                console.log(`[MCP Client] 执行 GET_ELEMENT_INFO, selector=${data.selector}`);
                result = await getElementInfo(data);
                break;

            case 'PREPARE_ROUTE_ACCESS':
                console.log('[MCP Client] 执行 PREPARE_ROUTE_ACCESS');
                result = await prepareRouteAccess();
                break;

            case 'SCAN_ROUTE_FOR_API':
                console.log(`[MCP Client] 执行 SCAN_ROUTE_FOR_API, route=${data.route}`);
                result = await scanRouteForAPI(data);
                break;

            // ===== 新增：加密分析命令 =====
            case 'ENABLE_ENCRYPTION_HOOKS':
                console.log('[MCP Client] 执行 ENABLE_ENCRYPTION_HOOKS');
                result = await enableEncryptionHooks();
                break;

            case 'INJECT_ENCRYPTION_CAPTURE':
                console.log('[MCP Client] 执行 INJECT_ENCRYPTION_CAPTURE');
                result = await injectEncryptionCapture();
                break;

            case 'GET_CAPTURED_ENCRYPTION':
                console.log('[MCP Client] 执行 GET_CAPTURED_ENCRYPTION');
                result = await getCapturedEncryption();
                break;

            case 'ANALYZE_PAGE_ENCRYPTION':
                console.log('[MCP Client] 执行 ANALYZE_PAGE_ENCRYPTION');
                result = await analyzePageEncryption();
                break;

            case 'AUTO_LOGIN_AND_CAPTURE':
                console.log('[MCP Client] 执行 AUTO_LOGIN_AND_CAPTURE');
                result = await autoLoginAndCapture(data);
                break;

            case 'DECRYPT_RSA':
                console.log(`[MCP Client] 执行 DECRYPT_RSA, encryptedLength=${data.encryptedData ? data.encryptedData.length : 0}`);
                result = await decryptRSA(data);
                break;

            case 'EXTRACT_KEYS_FROM_JS':
                console.log(`[MCP Client] 执行 EXTRACT_KEYS_FROM_JS, jsUrl=${data.jsUrl}`);
                result = await extractKeysFromJS(data);
                break;

            // ===== 智能化功能命令 =====
            case 'DETECT_ANTI_DEBUG':
                console.log('[MCP Client] 执行 DETECT_ANTI_DEBUG');
                result = await detectAntiDebug();
                break;

            case 'AUTO_BYPASS_ANTI_DEBUG':
                console.log('[MCP Client] 执行 AUTO_BYPASS_ANTI_DEBUG');
                result = await autoBypassAntiDebug();
                break;

            case 'ANALYZE_API_SIGNATURE':
                console.log('[MCP Client] 执行 ANALYZE_API_SIGNATURE');
                result = await analyzeAPISignature();
                break;

            case 'EXTRACT_VUE_DATA':
                console.log(`[MCP Client] 执行 EXTRACT_VUE_DATA, selector=${data.selector}`);
                result = await extractVueData(data);
                break;

            case 'EXTRACT_REACT_DATA':
                console.log(`[MCP Client] 执行 EXTRACT_REACT_DATA, selector=${data.selector}`);
                result = await extractReactData(data);
                break;

            case 'ANALYZE_AUTHENTICATION':
                console.log('[MCP Client] 执行 ANALYZE_AUTHENTICATION');
                result = await analyzeAuthentication();
                break;

            case 'GET_PAGE_FORMS':
                console.log('[MCP Client] 执行 GET_PAGE_FORMS');
                result = await getPageForms();
                break;

            case 'AUTO_FILL_FORM':
                console.log(`[MCP Client] 执行 AUTO_FILL_FORM, formIndex=${data.formIndex}`);
                result = await autoFillForm(data);
                break;

            case 'INJECT_WS_MONITOR':
                console.log('[MCP Client] 执行 INJECT_WS_MONITOR');
                result = await injectWSMonitor();
                break;

            case 'GET_WS_MESSAGES':
                console.log(`[MCP Client] 执行 GET_WS_MESSAGES, limit=${data.limit}`);
                result = await getWSMessages(data);
                break;

            case 'EXTRACT_PAGE_DATA':
                console.log(`[MCP Client] 执行 EXTRACT_PAGE_DATA, selectors数=${data.selectors ? Object.keys(data.selectors).length : 0}`);
                result = await extractPageData(data);
                break;

            case 'EXTRACT_TABLE_DATA':
                console.log(`[MCP Client] 执行 EXTRACT_TABLE_DATA, selector=${data.selector}`);
                result = await extractTableData(data);
                break;

            // ===== Burp 格式请求 =====
            case 'CLICK_AND_CAPTURE':
                console.log(`[MCP Client] 执行 CLICK_AND_CAPTURE, selector=${data.selector}`);
                result = await clickAndCapture(data);
                break;

            case 'GET_NETWORK_REQUESTS_BURP':
                console.log(`[MCP Client] 执行 GET_NETWORK_REQUESTS_BURP, filter=${data.filter}`);
                result = await getNetworkRequestsBurp(data);
                break;

            case 'CLICK_AND_GET_BURP':
                console.log(`[MCP Client] 执行 CLICK_AND_GET_BURP, selector=${data.selector}`);
                result = await clickAndGetBurp(data);
                break;

            case 'LOGIN_AND_GET_BURP':
                console.log('[MCP Client] 执行 LOGIN_AND_GET_BURP');
                result = await loginAndGetBurp(data);
                break;

            // ===== 敏感数据检测功能 =====
            case 'GET_NETWORK_REQUESTS_WITH_BODY':
                console.log(`[MCP Client] 执行 GET_NETWORK_REQUESTS_WITH_BODY, filter=${data.filter}`);
                result = await getNetworkRequestsWithBody(data);
                break;

            case 'GET_PAGE_CONTENT':
                console.log('[MCP Client] 执行 GET_PAGE_CONTENT');
                result = await getPageContent();
                break;

            case 'START_SENSITIVE_MONITOR':
                console.log(`[MCP Client] 执行 START_SENSITIVE_MONITOR, patterns=${JSON.stringify(data.patterns)}`);
                result = await startSensitiveMonitor(data);
                break;

            case 'GET_SENSITIVE_ALERTS':
                console.log(`[MCP Client] 执行 GET_SENSITIVE_ALERTS, severityFilter=${data.severityFilter}`);
                result = await getSensitiveAlerts(data);
                break;

            // ===== 请求头管理功能 =====
            case 'GET_HEADERS_CONFIG':
                console.log('[MCP Client] 执行 GET_HEADERS_CONFIG');
                result = await getHeadersConfig();
                break;

            case 'CREATE_HEADER_GROUP':
                console.log(`[MCP Client] 执行 CREATE_HEADER_GROUP, name=${data.name}`);
                result = await createHeaderGroup(data);
                break;

            case 'DELETE_HEADER_GROUP':
                console.log(`[MCP Client] 执行 DELETE_HEADER_GROUP, groupId=${data.groupId}`);
                result = await deleteHeaderGroup(data);
                break;

            case 'SWITCH_HEADER_GROUP':
                console.log(`[MCP Client] 执行 SWITCH_HEADER_GROUP, groupId=${data.groupId}`);
                result = await switchHeaderGroup(data);
                break;

            case 'ADD_HEADER':
                console.log(`[MCP Client] 执行 ADD_HEADER, groupId=${data.groupId}, name=${data.name}`);
                result = await addHeader(data);
                break;

            case 'UPDATE_HEADER':
                console.log(`[MCP Client] 执行 UPDATE_HEADER, groupId=${data.groupId}, headerId=${data.headerId}`);
                result = await updateHeader(data);
                break;

            case 'DELETE_HEADER':
                console.log(`[MCP Client] 执行 DELETE_HEADER, groupId=${data.groupId}, headerId=${data.headerId}`);
                result = await deleteHeader(data);
                break;

            case 'TOGGLE_HEADER':
                console.log(`[MCP Client] 执行 TOGGLE_HEADER, groupId=${data.groupId}, headerId=${data.headerId}, enabled=${data.enabled}`);
                result = await toggleHeader(data);
                break;

            case 'BATCH_UPDATE_HEADERS':
                console.log(`[MCP Client] 执行 BATCH_UPDATE_HEADERS, groupId=${data.groupId}, headers数=${data.headers ? data.headers.length : 0}`);
                result = await batchUpdateHeaders(data);
                break;

            // ===== 智能场景协调功能 =====
            case 'BATCH_ENABLE_SCRIPTS':
                console.log(`[MCP Client] 执行 BATCH_ENABLE_SCRIPTS, scriptIds=${JSON.stringify(data.scriptIds)}, enabled=${data.enabled}`);
                result = await batchEnableScripts(data);
                break;

            case 'DISABLE_ALL_SCRIPTS':
                console.log('[MCP Client] 执行 DISABLE_ALL_SCRIPTS');
                result = await disableAllScripts(data);
                break;

            default:
                console.error(`[MCP Client] ❌ 未知命令: ${type}`);
                throw new Error(`未知命令: ${type}`);
        }

        console.log(`[MCP Client] ✅ 命令 ${type} 执行完成`);
        sendResponse(requestId, result);
    } catch (error) {
        console.error(`[MCP Client] ❌ 命令 ${type} 处理失败:`, error.message, error.stack);
...
async function configureHook({ scriptId, config }) {
    ...
    if (config.rotate !== undefined) {
        hookConfig.rotate = config.rotate;
    }
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    console.log(`[MCP Client] 收到content消息: type=${message.type}, sender=${sender ? sender.tabId : 'none'}`);

    // 处理Hook数据
    if (message.type === 'MCP_HOOK_DATA') {
        console.log(`[MCP Client] MCP_HOOK_DATA: scriptId=${message.data.scriptId}, count=${message.data.buffer ? message.data.buffer.length : 0}`);
